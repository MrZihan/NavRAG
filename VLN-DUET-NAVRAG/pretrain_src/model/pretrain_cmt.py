...
class GlocalTextPathCMTPreTraining(BertPreTrainedModel):
    ...
    def forward_sap(
        ...
        if compute_loss:
            ...
            losses = torch.cat([global_losses[global_act_labels!=0],scale_value*global_losses[global_act_labels==0]],dim=0) + torch.cat([local_losses[local_act_labels!=0],scale_value*local_losses[local_act_labels==0]],dim=0) + torch.cat([fused_losses[global_act_labels!=0],scale_value*fused_losses[global_act_labels==0]],dim=0)
            return losses
        else:
