...
class ReverieTextPathData(object):
    def __init__(
        ...
        if type(connectivity_dir) == type(["",""]):
            self.graphs, self.shortest_distances, self.shortest_paths = {}, {}, {}
            for item in connectivity_dir:
                graphs, shortest_distances, shortest_paths = load_nav_graphs(item)
                self.graphs.update(graphs)
                self.shortest_distances.update(shortest_distances)
