...
def process_features(proc_id, out_queue, scanvp_list, args, prompt, mp3d_annotation):
    ...
    for scan_id, viewpoint_id in scanvp_list:
        if scan_id in mp3d_annotation and viewpoint_id in mp3d_annotation[scan_id] and len(mp3d_annotation[scan_id][viewpoint_id])==6: # only store 6 views
            continue
        ...
        for ix in range(VIEWPOINT_SIZE):
            ...
            #print(scan_id, viewpoint_id, habitat_position, habitat_rotation) # 17DRP5sb8fy 10c252c90fa24ef3b698c6f54d984c5c 
            # [-5.48891019821167, 0.28508996963500977, -1.4484000205993652] [0.         0.25881905 0.         0.96592583]
...
def build_output_file(args, prompt):
    ...
    if os.path.exists("mp3d_view_annotation.json"):
        mp3d_annotation = json.load(open("mp3d_view_annotation.json","r"))
    else:
        mp3d_annotation = {}
    ...
    for proc_id in range(num_workers):
        ...
        process = mp.Process(
            ...
            args=(proc_id, out_queue, scanvp_list[sidx: eidx], args, prompt, mp3d_annotation)
        )
        process.start()
    ...
    while num_finished_workers < num_workers:
        ...
        if res is None:
            ...
            for scene_id in scene_annotation:
                if scene_id not in mp3d_annotation:
                    mp3d_annotation[scene_id] = {}

                for viewpoint_id in scene_annotation[scene_id]:
                    mp3d_annotation[scene_id][viewpoint_id] = scene_annotation[scene_id][viewpoint_id]

            num_finished_vps += 1
            progress_bar.update(num_finished_vps)

        if num_finished_vps % 500 == 0:
            with open("mp3d_view_annotation.json", "w") as file:
                json.dump(mp3d_annotation, file)
